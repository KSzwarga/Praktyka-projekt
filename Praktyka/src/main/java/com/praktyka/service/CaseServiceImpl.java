...
public class CaseServiceImpl implements CaseService{
    ...
    private Specification<Case> caseNumberLike(String surname){
        return (root, query, criteriaBuilder) -> {
            ...
            Predicate p1 = criteriaBuilder.like(caseDoc.join("outgoingCorrespondence", JoinType.LEFT)
                    .join("correspondentId", JoinType.LEFT).get("surname"), surname);
            Predicate p2 = criteriaBuilder.like(caseDoc.join("incomingCorrespondence", JoinType.LEFT)
                    .join("correspondentId", JoinType.LEFT).get("surname"), surname);
